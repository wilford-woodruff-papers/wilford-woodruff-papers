...
class StripLanguageTag
{
    public function __invoke()
    {
        return function ($subject) {
            return $this->replaceInlineLanguageTags(
                $this->removePageLanguageTag($subject)
            );
        };
    }

    private function removePageLanguageTag($subject)
    {
        return Str::of($subject)
            ->replaceMatches('/\[Language:.*?\][\s|\n]*/', '')
            ->trim();
    }

    private function replaceInlineLanguageTags($subject)
    {
        return Str::of($subject)
                    ...
                    case 1:
                        return `
                            <div
                                ...
                                <div x-ref="content" class="px-3 py-1 text-xs rounded-full cursor-pointer text-neutral-500 bg-neutral-100">${$parts->first()}</div>
