<li class="grid grid-cols-7 py-4">
    <div class="col-span-1 px-2">
        <a class="col-span-1 my-2 mx-auto w-20 h-auto" href="{{ route('pages.show', ['item' => $quote->page->parent, 'page' => $quote->page]) }}">
    ...
    <div class="col-span-6 py-2 px-4">
        <p class="pb-1 text-lg font-medium capitalize text-secondary">
            <a href="{{ route('pages.show', ['item' => $quote->page->parent, 'page' => $quote->page]) }}">Page {{ $quote->page->order }}</a>
        </p>
        <div class="flex gap-x-3 ml-2 text-base font-medium">
            <div>
                <span class="text-gray-600">Part of </span>
                <span class="text-secondary">
                    <a href="{{ route('documents.show', ['item' => $quote->page->parent]) }}">{{ \Illuminate\Support\Str::of($quote->page->parent?->name)->replaceMatches('/\[.*?\]/', '')->trim() }}</a>
                </span>
            </div>
            <div>
                @auth()
